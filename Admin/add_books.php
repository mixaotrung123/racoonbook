...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Lấy nguyên chuỗi đường dẫn ảnh nhập từ form, không validate
        $img_path = isset($_POST['img_url']) ? trim($_POST['img_url']) : null;
        if ($img_path === '') {
            $img_path = null;
        }
        ...
        if (empty($errors)) {
            $stmt = $pdo->prepare("INSERT INTO sach (ten_sach, tac_gia, nha_xuat_ban, gia, so_luong, mo_ta, uu_dai_id, danh_muc_id, img) 
                VALUES (:ten_sach, :tac_gia, :nha_xuat_ban, :gia, :so_luong, :mo_ta, :uu_dai_id, :danh_muc_id, :img)");
            $stmt->execute([
                ':ten_sach' => $ten_sach,
                ':tac_gia' => $tac_gia,
...
// Hàm chuyển dấu \ thành / khi hiển thị ảnh trên web
...
<body>
    <div class="sidebar">
        ...
        $col = [
            ...
            ["label" => "Báo cáo", "route" => "\Hethongbansach\Admin\RSeports.php"],
    ...
    <main class="content">
        ...
        <form class="add-book-form" method="POST" action="">
            ...
            <input type="text" id="ten_sach" name="ten_sach" required value="<?= htmlspecialchars($_POST['ten_sach'] ?? '') ?>" />

            <label for="tac_gia">Tác giả</label>
            <input type="text" id="tac_gia" name="tac_gia" value="<?= htmlspecialchars($_POST['tac_gia'] ?? '') ?>" />

            <label for="nha_xuat_ban">Nhà xuất bản</label>
            <input type="text" id="nha_xuat_ban" name="nha_xuat_ban" value="<?= htmlspecialchars($_POST['nha_xuat_ban'] ?? '') ?>" />

            <label for="gia">Giá *</label>
            <input type="number" id="gia" name="gia" min="0" step="0.01" required value="<?= htmlspecialchars($_POST['gia'] ?? '') ?>" />

            <label for="so_luong">Số lượng *</label>
            <input type="number" id="so_luong" name="so_luong" min="0" required value="<?= htmlspecialchars($_POST['so_luong'] ?? '') ?>" />

            <label for="mo_ta">Mô tả</label>
            <textarea id="mo_ta" name="mo_ta"><?= htmlspecialchars($_POST['mo_ta'] ?? '') ?></textarea>
            ...
            <select id="uu_dai_id" name="uu_dai_id">
                ...
                <?php foreach($uu_dais as $ud): ?>
                    <option value="<?= $ud['uu_dai_id'] ?>" <?= (isset($_POST['uu_dai_id']) && $_POST['uu_dai_id'] == $ud['uu_dai_id']) ? 'selected' : '' ?>>
            ...
            <select id="danh_muc_id" name="danh_muc_id">
                ...
                <?php foreach($danh_mucs as $dm): ?>
                    <option value="<?= $dm['danh_muc_id'] ?>" <?= (isset($_POST['danh_muc_id']) && $_POST['danh_muc_id'] == $dm['danh_muc_id']) ? 'selected' : '' ?>>
            ...
            <input type="text" id="img_url" name="img_url" placeholder="Nhập đường dẫn ảnh" value="<?= htmlspecialchars($_POST['img_url'] ?? '') ?>" />
